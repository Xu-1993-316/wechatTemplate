...
var defaultTitle = document.title,
  ...
  install = function (Vue, options) {
    var root,
      router,
      defaultForward = options && options.defaultForward ? options.defaultForward : 'forward',
      forwardAnimation = null,
      backwardAnimation = null,
      defaultBackward = options && options.defaultBackward ? options.defaultBackward : 'backward',
      pageData = {},
      switchLock = false;
    ...
    function pageDelegate(comp, _callback) {
      if (comp.$options.isPage) {
        _callback.call(comp, pageData);
      }
    }

    function saveScrollTop(context) {
      pageDelegate(context, function () {
        var pageName = this.$options.name;
        var $main = this.$el.getElementsByTagName('MAIN')[0];
        var scrollTopList = this.$sessionStorage.getItem('scrollTopList') || {};
        if (!$main) return;

        scrollTopList[pageName] = $main.scrollTop;
        this.$sessionStorage.setItem('scrollTopList', scrollTopList)
      });
    }

    function setScrollTop(context) {
      pageDelegate(context, function (data) {
        var pageName = this.$options.name;
        var $main = this.$el.getElementsByTagName('MAIN')[0];
        if (!$main) return;

        var scrollTopList = this.$sessionStorage.getItem('scrollTopList') || {};
        $main.scroll(0, scrollTopList[pageName]);
      });
    }
