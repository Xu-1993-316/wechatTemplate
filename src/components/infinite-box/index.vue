<template>
  <div class="infiniteWrapper"
       :style="{ height: wrapperHeight, overflow: hideTips && !isIos ? 'hidden' : 'visible', transform: `translate3d(0, ${translateY}px, 0)` }">
    <div v-if="downRefresh"
         class="tips"
         :style="{ height: `${topTipsHeight}px`, marginTop: `-${topTipsHeight}px` }">
      <slot name="topTips">
    ...
    <div class="infiniteContainer"
      ...
      <p class="endTips"
         v-if="noData">没有更多内容...</p>
    </div>
    <div v-if="upLoading"
         class="tips"
         :style="{ height: `${bottomTipsHeight}px`, marginBottom: `-${bottomTipsHeight}px` }">
